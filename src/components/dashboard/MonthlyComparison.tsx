...
function ChangeIndicator({ change }: { change: number | null }) {
  ...
  const isUp = change > 0;
  const color = isUp ? "var(--destructive)" : "var(--success)";
  return (
...
function IncomeChangeIndicator({ change }: { change: number | null }) {
  if (change === null) return <span className="text-xs text-[var(--muted-foreground)]">N/A</span>;
  const isUp = change > 0;
...
export function MonthlyComparison() {
  ...
  const thisExpense = getTotalExpense(thisMonthTx);
  const lastExpense = getTotalExpense(lastMonthTx);
  const thisIncome = getTotalIncome(thisMonthTx);
  const lastIncome = getTotalIncome(lastMonthTx);

  const expenseChange = getPercentChange(thisExpense, lastExpense);
  const incomeChange = getPercentChange(thisIncome, lastIncome);
  ...
  const items = [
    {
      ...
      changeEl: <ChangeIndicator change={expenseChange} />,
    ...
    },
    {
      ...
      current: thisIncome,
      previous: lastIncome,
      changeEl: <IncomeChangeIndicator change={incomeChange} />,
      color: "var(--success)",
    },
  ];
